...
class AdventOfCode extends BaseAdventOfCode {
  ...
  getPossibleMoves(i, j, last_i, last_j) {
    ...
    for (const possibleMove of [
      ...
      const [possible_i, possible_j, allowedValues, direction] = possibleMove

      if (!this.input?.[possible_i]?.[possible_j]) continue

      if (possible_i === last_i && possible_j === last_j) {
        continue
      }
  ...
  callback() {
    ...
    startingPositionLoop: for (let i = 0; i < this.input.length; i++) {
      for (let j = 0; j < this.input[i].length; j++) {
        ...
        for (const allowedValue of Object.keys(this.allowedDirectionsMap)) {
          ...
          if (
            allowedDirection.sort().join("-") === directions.sort().join("-")
          ) {
